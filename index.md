# Попов Артём Сергеевич
**Python Backend Developer**  
Делаю REST API backend, занимался мобильной разработкой и готов рассмотреть предложения по C++ проектам.

---

## 📞 Контакты

* **Телефон:** +7 (922) 273-04-02
* **Email:** qwingel71@yandex.ru
* **Telegram:** [@koreanrating](https://t.me/koreanrating)
* **GitHub:** [github.com/qwingel](https://github.com/qwingel)
* **Санкт-Петербург**

---

## 🛠 Технические навыки

**Backend:** Django, Flask, REST API  
**Python** — основной язык  
**БД:** PostgreSQL, MySQL, SQLite  
**Инструменты:** Git, Docker, HTML/CSS, JavaScript  
**Прочее:** C++ (C++17, STL, ООП), Java, Kotlin, ООП, паттерны проектирования, многопоточность, работа с памятью, WinAPI, инжектирование кода

---

## 🚀 Основные проекты

### **LisaVibes** — Музыкальный сервис
*Private | В разработке*

Полнофункциональный музыкальный сервис с системой авторизации и медиа-библиотекой.

Реализовано:
- Система аутентификации с подтверждением email
- Восстановление пароля через email
- Работа с файловым хранилищем и обработка изображений
- JWT-токены для авторизации
- REST API на Django Rest Framework

**Стек:** Python, Django, PostgreSQL, boto3, Pillow, JWT, REST API

---

### **Random Quotes** — Веб-приложение
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/it-solution-test) [![Demo](https://img.shields.io/badge/Live-Demo-green)](https://itsis.it-solution.ru/meet_matrix/view/?secret=t6p3q404fiinb08gmw8urwg9ep1wyijjv3sw6fb68q6duqclc0)

Веб-приложение для работы с цитатами. Пользователи могут просматривать случайные цитаты, добавлять новые и оценивать.

**Стек:** Django, SQLite, HTML/CSS, JavaScript

---

### **CSMonitoringBot** — Telegram бот
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/CSMonitoringBot)

Telegram бот для мониторинга состояния игровых серверов через Steam API.

**Стек:** Python, pyTelegramBotAPI, python-a2s, python-dotenv, pyyaml

---

### **BeautyBans** — Система управления наказаниями для CS серверов
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/BeautyBans)

Веб-система для управления наказаниями (баны, муты, гаги) и правами администраторов на игровых серверах Counter-Strike.

**Стек:** Python, Django, PostgreSQL, Nginx, Docker, REST API

## 🚀 Остальные проекты

### **BeautyBans_AMXX** — Плагин для AMX Mod X
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/BeautyBans_AMXX)

Серверный плагин для Counter-Strike 1.6, обеспечивающий интеграцию с системой BeautyBans. Синхронизирует наказания и права администраторов с облачным бэкендом через HTTP API.

**Стек:** Pawn, AMX Mod X 1.9+, ReAPI, easy_http, JSON, REST API

---

### **intersectionCalculator** — Вычисление изолиний триангулированной поверхности
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/intersectionCalculator)

Алгоритм вычисления контурных линий (изолиний) на триангулированной поверхности. Программа находит пересечение сетки треугольников с горизонтальной плоскостью и собирает замкнутые полилинии.

Реализовано:
- Алгоритм Marching Triangles с эйлеровыми циклами
- Обработка вырожденных случаев (плато, хребты, седловые точки)
- Построение графа рёбер и сборка замкнутых контуров

**Стек:** C++17, STL, алгоритмы вычислительной геометрии

---

### **AutoBattler** — C++ прототип
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/LestaGames-Test)

Прототип пошагового автобаттлера по техническому заданию от Lesta Games. Реализована система создания персонажей, автоматических боёв и прокачки с использованием ООП.

**Стек:** C++17, STL, умные указатели, паттерны "Фабрика", наследование, полиморфизм

---

### **KZ-Stats-Loader & KZ-Internal-Stats** — Система мониторинга
*Private*

Комплексная система для мониторинга игрового процесса, состоящая из загрузчика и внедряемой библиотеки.

**KZ-Stats-Loader:**
- Загрузка и внедрение DLL в целевой процесс
- Работа с WinAPI и запросы через cURL

**KZ-Internal-Stats:**
- Оверлей для отображения статистики в реальном времени
- Чтение и модификация памяти процесса

**Стек:** C++, WinAPI, ImGui, OpenGL, cURL, инжектирование кода

---

### **SkyLine** — Мобильное приложение
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/myitschoolsamsung)

Прототип мобильного приложения для авиакомпании с клиент-серверной архитектурой. Приложение не предназначено для покупки реальных билетов.

**Стек:** Java, Flask (Python), SQLite, Retrofit, SharedPreferences

---

### **Horse Racing** — Мобильное приложение
[![GitHub](https://img.shields.io/badge/GitHub-Repository-blue)](https://github.com/qwingel/HorseRacing)

Мобильное приложение, моделирующее лошадиные скачки. Все результаты записываются в локальную БД.

**Стек:** Kotlin, SQLite, Fragments, RecyclerView

---

### **ML-basics-algorithms** — Алгоритмы

Реализация и примеры использования основных алгоритмов машинного обучения

**Стек:** Python, NumPy, Scikit-learn

---

### **predictor_runtime** — Пайплайн предсказаний CS2
*Private*

Автоматический пайплайн предсказаний CS2: парсинг HLTV → прогноз → публикация в Telegram.

Runtime-часть проекта — делает предсказания на основе обученной модели LightGBM. Парсит расписание матчей с hltv.org, генерирует прогнозы для всех карт, парсит коэффициенты Winline, рассчитывает EV и рекомендации по ставкам (Kelly criterion), публикует отчёт в Telegram.

Ключевая функциональность:

Парсинг HLTV через Selenium (матчи топ‑30 команд)

Предсказание исхода всех 7 карт и общего победителя матча

Температурная калибровка вероятностей (Temperature scaling)

Симметричная модель: P(A,B) = 1 - P(B,A)

Расчёт оптимального размера ставки по дробному Келли (25%)

Автоматическая проверка точности прогнозов на следующий день

**Стек:** Python, LightGBM, pandas, numpy, scikit-learn, Selenium, undetected-chromedriver, requests, SQLite

---

### **Predictor Bot** — Telegram Subscription Bot
*Private*

Telegram-бот для продажи доступа к VIP-каналу через DonationAlerts.

Возможности: выбор тарифа (₽/$), проверка статуса подписки, автоматическая выдача и отзыв доступа при оплате/истечении подписки, уведомления админу.

Архитектура:

Aiogram 3 — обработка команд и сообщений

HTTP-сервер (aiohttp) — приём вебхуков от DonationAlerts

Планировщик (apscheduler) — проверка истёкших подписок каждый час

SQLite + aiosqlite — хранение данных подписок

**Стек:** Python, aiogram, aiohttp, apscheduler, aiosqlite, SQLite, DonationAlerts API

---

## 🎓 Образование

**Физико-Математический Лицей (ФМЛИ)** - Сыктывкар

**РГПУ им. Герцена, Бакалавриат, 3 курс** — Информационные системы и технологии (интеллектуальные информационные системы)

**Дополнительное образование:**
- МФТИ — C++
- IT Школа Samsung — Мобильная разработка на Java
- VK Образование — Алгоритмы и структуры данных
- Stepik — Java, PostgreSQL, Docker
- Курс по основам Machine Learning

## 💭 О себе

Мне нравятся задачи, которые требуют понимания, возможно, интересного или нестандартного решения, люблю и разбираюсь в математике. 
Фокусируюсь на ML/backend-разработке.

---

*Резюме обновлено: июль 2026*  
*📧 Открыт к интересным предложениям и сотрудничеству!*
